...
-- ────────────────────────────────────────────────────────────
-- ÉTAPE 9 : MENU LE CENTRE (pizza, fruits de mer, international)
INSERT INTO public.categories (nom, description, emoji, ordre, actif) VALUES
('Pizzas','Pizzas cuites au feu de bois','🍕',1,true),
('Fruits de Mer','Crevettes, homard, calamars, plateau','🦞',2,true),
('Grillades','Viandes et volailles grillees','🥩',3,true),
('Pates','Pates maison et risottos','🍝',4,true),
('Salades','Salades composees fraiches','🥗',5,true),
('Desserts','Desserts et douceurs','🍰',6,true),
('Boissons','Vins, bieres, cocktails, softs','🥤',7,true)
ON CONFLICT DO NOTHING;

INSERT INTO public.produits (nom, description, prix, categorie_id, disponible, ordre) VALUES
('Pizza Margherita','Tomate, mozzarella, basilic frais',18.00,(SELECT id FROM categories WHERE nom='Pizzas'),true,1),
('Pizza 4 Fromages','Mozzarella, gorgonzola, chevre, parmesan',22.00,(SELECT id FROM categories WHERE nom='Pizzas'),true,2),
('Pizza Fruits de Mer','Crevettes, calamars, moules, sauce tomate',25.00,(SELECT id FROM categories WHERE nom='Pizzas'),true,3),
('Pizza Vegetarienne','Legumes grilles, mozzarella, roquette',20.00,(SELECT id FROM categories WHERE nom='Pizzas'),true,4),
('Pizza Jambon-Champignons',NULL,20.00,(SELECT id FROM categories WHERE nom='Pizzas'),true,5),
('Plateau Fruits de Mer','Crevettes + homard + calamars pour 2',45.00,(SELECT id FROM categories WHERE nom='Fruits de Mer'),true,1),
('Crevettes Sautees a l ail','Crevettes geantes, ail, beurre, citron',25.00,(SELECT id FROM categories WHERE nom='Fruits de Mer'),true,2),
('Homard Grille','Sauce bearnaise ou beurre blanc',40.00,(SELECT id FROM categories WHERE nom='Fruits de Mer'),true,3),
('Calamars Frits','Panes dores, sauce tartare maison',18.00,(SELECT id FROM categories WHERE nom='Fruits de Mer'),true,4),
('Brochette de Crevettes','Marinees, grillees, legumes',22.00,(SELECT id FROM categories WHERE nom='Fruits de Mer'),true,5),
('Entrecote Grillee','300g, frites maison ou legumes',28.00,(SELECT id FROM categories WHERE nom='Grillades'),true,1),
('Poulet Roti au Four','Demi-poulet, herbes fraiches, legumes rotis',18.00,(SELECT id FROM categories WHERE nom='Grillades'),true,2),
('Cotes d agneau','Grillees, sauce menthe',32.00,(SELECT id FROM categories WHERE nom='Grillades'),true,3),
('Spaghetti Bolognaise','Viande hachee, sauce tomate fraiche',18.00,(SELECT id FROM categories WHERE nom='Pates'),true,1),
('Tagliatelles Fruits de Mer','Crevettes, calamars, creme',24.00,(SELECT id FROM categories WHERE nom='Pates'),true,2),
('Risotto Champignons','Champignons sauvages, parmesan, beurre',22.00,(SELECT id FROM categories WHERE nom='Pates'),true,3),
('Salade Cesar','Poulet grille, parmesan, croutons',12.00,(SELECT id FROM categories WHERE nom='Salades'),true,1),
('Salade Nicoise','Thon, oeufs, olives, anchois, tomates',12.00,(SELECT id FROM categories WHERE nom='Salades'),true,2),
('Salade Chevre Chaud','Fromage de chevre, noix, miel, roquette',14.00,(SELECT id FROM categories WHERE nom='Salades'),true,3),
('Tiramisu Maison',NULL,10.00,(SELECT id FROM categories WHERE nom='Desserts'),true,1),
('Fondant Chocolat','Coulant, glace vanille',10.00,(SELECT id FROM categories WHERE nom='Desserts'),true,2),
('Panna Cotta','Coulis de fruits rouges',9.00,(SELECT id FROM categories WHERE nom='Desserts'),true,3),
('Vin Rouge (verre)',NULL,8.00,(SELECT id FROM categories WHERE nom='Boissons'),true,1),
('Vin Blanc (verre)',NULL,8.00,(SELECT id FROM categories WHERE nom='Boissons'),true,2),
('Cocktail Maison',NULL,9.00,(SELECT id FROM categories WHERE nom='Boissons'),true,3),
('Biere Importee 33cl',NULL,5.00,(SELECT id FROM categories WHERE nom='Boissons'),true,4),
('Jus de Fruit Frais',NULL,5.00,(SELECT id FROM categories WHERE nom='Boissons'),true,5),
('Eau Minerale 75cl',NULL,3.00,(SELECT id FROM categories WHERE nom='Boissons'),true,6)
ON CONFLICT DO NOTHING;
